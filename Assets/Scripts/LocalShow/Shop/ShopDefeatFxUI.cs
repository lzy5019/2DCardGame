...
public class ShopDefeatFxUI : MonoBehaviour
{
    ...
    public IEnumerator PlayDefeatRoutine(Sprite cardSprite)
    {
        ...
        if (cardImage == null)
        {
            Debug.LogWarning("ShopDefeatFxUI: missing card Image, unable to play defeat effect.");
            yield break;
        }

        if (!PrepareRuntimeMaterial())
        {
            Debug.LogWarning("ShopDefeatFxUI: missing defeat material template, unable to play defeat effect.");
            yield break;
        }
